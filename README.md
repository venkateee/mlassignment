...
## a. Problem statement
This project implements and compares six supervised classification algorithms
on a single multi-class dataset and exposes them through an interactive
Streamlit web application. The app lets a user upload a test CSV, select any
trained model, and immediately view its evaluation metrics, confusion matrix,
and classification report — demonstrating a complete machine learning workflow
from model training through to deployment.
...
| Decision Tree | Weakest of the tree/margin group (0.827); a single tree overfits its training split and generalises poorly. |
